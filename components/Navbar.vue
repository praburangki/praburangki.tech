<script>
export default {
  props: {
    navIsVisible: {
      type: Boolean,
      default: false
    }
  },
  data: () => ({
    navLinks: [
      {
        text: 'Home',
        path: '/'
      },
      {
        text: 'Portfolio',
        path: '/portfolios'
      },
      {
        text: 'Skill',
        path: '/skills'
      },
      {
        text: 'Blog',
        path: '/blogs'
      }
    ]
  }),
  methods: {
    currentRoute(route) {
      return this.$route.path === route;
    }
  }
};
</script>

<template lang="pug">
  div
    header.topNav(:class="{ navIsVisible: navIsVisible }")
      a.toggleNav(@click="$emit('toggleNavbar')")
        |Menu
        span
    nav.navContainer(:class="{ navIsVisible: navIsVisible }")
      .navList.columns.is-gapless.is-mobile
        nuxt-link.column.navItem(
          v-for="navLink in navLinks"
          :key="navLink.text"
          @click.native="$emit('toggleNavbar')"
          :to="navLink.path"
          :class="{ selected: currentRoute(navLink.path) }"
        )
          |{{ navLink.text }}
          span.navMarker
</template>

<style lang="scss" scoped>
.topNav {
  height: 80px;
  width: 90%;
  margin: 0 auto;
  transition: transform 0.5s;
  transform: translateZ(0);
  backface-visibility: hidden;

  &.navIsVisible {
    transform: translateY(80px);
  }

  @media only screen and (min-width: 768px) {
    &.navIsVisible {
      transform: translateY(170px);
    }
  }
}

.toggleNav {
  position: relative;
  float: right;
  height: 45px;
  width: 45px;
  margin-top: 18px;
  overflow: hidden;
  text-indent: 100%;
  white-space: nowrap;
  color: transparent;

  span,
  span::before,
  span::after {
    position: absolute;
    width: 28px;
    height: 3px;
    background-color: #fff;
    backface-visibility: hidden;
  }

  span {
    top: 21px;
    left: 8px;
    transition: background 0.2s 0.5s;

    &::before,
    &:after {
      content: '';
      left: 0;
      transition: transform 0.2s 0.5s;
    }

    &::before {
      bottom: 8px;
    }

    &::after {
      top: 8px;
    }
  }

  .navIsVisible & {
    span {
      background-color: rgba(#fff, 0);

      &::before,
      &::after {
        background-color: rgba(#fff, 1);
      }

      &::before {
        transform: translateY(8px) rotate(-45deg);
      }

      &::after {
        transform: translateY(-8px) rotate(45deg);
      }
    }
  }
}

.navContainer {
  position: fixed;
  top: 0;
  left: 0;
  height: 80px;
  width: 100%;
  background-color: #151515;
  visibility: hidden;
  perspective: 1000px;
  transform: translateY(-100%);
  transition: transform 0.5s 0s, visibility 0s 0.5s;

  &.navIsVisible {
    visibility: visible;
    transform: translateY(0);
    transition: transform 0.5s 0s, visibility 0.5s 0s;
  }

  @media only screen and (min-width: 768px) {
    height: 170px;
  }
}

.navList {
  position: relative;
  height: 100%;
  background-color: #151515;

  transform: translateZ(0);
  backface-visibility: hidden;
  transform-origin: center bottom;
  transform: rotateX(90deg);
  transition: transform 0.5s;

  &::after {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    height: 100%;
    width: 100%;
    background-color: #000;
    opacity: 1;
    visibility: visible;
    transition: opacity 0.5s 0s, visibility 0.5s 0s;
  }

  .navItem {
    display: flex;
    justify-content: center;
    align-items: center;
    position: relative;
    transition: background-color 0.2s;
    color: #fff;
    &:hover {
      background-color: #d73938;
    }

    &.selected {
      background-color: #2b2b2b;

      .navMarker {
        position: absolute;
        bottom: 0;
        left: 0;
        height: 3px;
        width: 100%;
        transform-origin: center bottom;
        transform: translateZ(0) rotateX(90deg);
        transition: transform 0.5s, left 0.5s, color 0.5s, background-color 0.5s;
        background-color: #d73938;

        &::before {
          content: '';
          position: absolute;
          bottom: 3px;
          left: 50%;
          right: auto;
          transform: translateX(-50%);
          height: 0;
          width: 0;
          border: 10px solid transparent;
          border-bottom-color: #d73938;
        }

        .navIsVisible & {
          transform: translateZ(0);
        }
      }
    }

    @media only screen and (min-width: 768px) {
      font: {
        size: 1.8rem;
        weight: 600;
      }
      -webkit-font-smoothing: antialiased;
      -moz-osx-font-smoothing: grayscale;
    }
  }

  .navIsVisible & {
    transform: translateZ(0);

    &::after {
      opacity: 0;
      visibility: hidden;
      transition: opacity 0.5s 0s, visibility 0s 0.5s;
    }
  }
}
</style>
